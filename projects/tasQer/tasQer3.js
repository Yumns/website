    ...
    for (i in allTasks) {
        ...
        if (allTasks[i].status === "incomplete") {
            ...
            <div class="row" style="text-decoration:none;">
                <div style="text-align: right" class="col-1">${Number(i) + 1}.</div>
        ...
        document.getElementById("display").innerHTML += `
        ${part1}
            <div id=${i + "summary"} onclick="editTask(this.id)" class="col-8">${allTasks[i].summary}</div>
            <div id=${i + "time"} onclick="editTask(this.id)" class="col-1">${Math.floor(allTasks[i].time / 60)}:${(allTasks[i].time % 60).toString().padStart(2, "0")}</div>
